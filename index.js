exports.calculateSchemaToRequest = (schema, user_metadata, multipage = true, max_attributes_per_page = 10) => {
  ...
  const getTopLevelAttributesFromProfile = (profile) =>
    Object.keys(profile).filter((itemKey) => typeof profile[itemKey] !== "object");
  const getTopLevelAttributesFromSchema = (raw_schema) =>
    Object.entries(raw_schema.properties)
      .filter(([_, value]) => !value.properties)
      .map(([key, _]) => key);
  const getNestedAttributesFromSchema = (raw_schema) =>
    Object.entries(raw_schema.properties)
      .filter(([_, value]) => value.properties)
      .map(([key, _]) => key);
  const determineUnnecessaryAttributes = (presentAttributes, requiredAttributes) =>
    presentAttributes.filter((attr) => requiredAttributes.includes(attr));
  ...
  const alreadyPresentTopLevelAttributes = user_metadata.customProfile
    ? getTopLevelAttributesFromProfile(user_metadata.customProfile)
    : [];
  const requiredTopLevelAttributes = getTopLevelAttributesFromSchema(workingCopy);
  const topLevelAttributesToRemove = determineUnnecessaryAttributes(
    alreadyPresentTopLevelAttributes,
    requiredTopLevelAttributes
  );
  ...
  for (const attr of requiredNestedAttributes) {
    const present =
      user_metadata?.customProfile && user_metadata?.customProfile[attr]
        ? getTopLevelAttributesFromProfile(user_metadata.customProfile[attr])
        : [];
